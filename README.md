...
- `load` executes `configs/model.py`; keep a simple `config = {...}` for small projects, or swap to `args = {...}; config = lambda a: ...` and `parents = [...]` when you need templates and composition (the callable receives a `Args` mapping).
